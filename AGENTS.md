# Directives & Contraintes d'Environnement — hebergement-guadeloupe-location.com

Ce document consigne l'architecture technique, les contraintes d'environnement, les directives de sécurité et les règles éditoriales pour le site **hebergement-guadeloupe-location.com** (hebergement-guadeloupe-location) hébergé sur Cloudflare Pages.
...
| :--- | :--- |
| **Domaine Apex** | `hebergement-guadeloupe-location.com` |
| **URL Cloudflare Pages** | `https://hebergement-guadeloupe-location.pages.dev` |
| **Dépôt GitHub** | `jc842/hebergement-guadeloupe-location` (branche principale : `main`) |
...
| **Matomo Site ID** | `60` |
| **Matomo MTM Container** | `container_OK8dU06g.js` (Container ID : `OK8dU06g`) |
| **Serveur Publicitaire** | Revive Adserver (`https://ads.les4h.fr/`) |
| **Revive Affiliate ID** | `110` |
| **Zones Publicitaires** | Header (`768`), In-Content (`769`), Mobile Sticky (`770`), Large Rectangle (`767`), Half Page (`766`), Skyscraper (`771`) |
...
- Ce Layout injecte au build statique le composant `Matomo.astro` avec le conteneur MTM dédié : **`container_OK8dU06g.js`**.
...
- Tout article doit obligatoirement comporter les 2 emplacements publicitaires intégrés dans le gabarit (`src/pages/blog/[...slug].astro`) :
  1. **Encart In-Content (300x250)** : `<ReviveSlot slot="inContent" />` positionné après les premiers paragraphes (Zone 769).
  2. **Encart Fin d'Article (336x280)** : `<ReviveSlot slot="largeRectangle" />` positionné avant les suggestions de maillage (Zone 767).
